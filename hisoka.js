...
module.exports = hisoka = async (hisoka, m, chatUpdate, store) => {
	...
	if (db.data.settings[botNumber].autobio) {
	  ...
	  // Anti Link
        ...
        if (!isBotAdmins) return m.reply(`لقد تم اكتشاف أنك ترسل رابط مجموعة ، آسف سيتم طردك!`)
        let gclink = (`https://chat.whatsapp.com/`+await hisoka.groupInviteCode(m.chat))
        let isLinkThisGc = new RegExp(gclink, 'i')
        let isgclink = isLinkThisGc.test(m.text)
        if (isgclink) return m.reply(`عععععععع`)
        if (isAdmins) return m.reply(`سوري`)
        if (isCreator) return m.reply(`عميييييييييييي`)
      ...
      return
        ...
        switch(command) {
            ...
            case 'join': {
                ...
                if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) throw 'الرابط'
                m.reply(mess.wait)
	...
	break
	    case 'setname': case 'setsubject': {
          ...
          case 'setppbot': {
                ...
                break
           case 'setppgroup': case 'setimage': case 'setppgrup': case 'setppgc': {
                if (!m.isGroup) throw mess.group
                if (!isAdmins) throw mess.admin
                if (!/image/.test(mime)) throw `أرسل الصورة ${prefix + command}`
                if (/webp/.test(mime)) throw `Kirim/Reply Image Dengan Caption ${prefix + command}`
                let media = await hisoka.downloadAndSaveMediaMessage(qmsg)
...
break
	    ...
	    break
           ...
           m.reply(mess.wait)
            ...
            break
            case 'stickerwm': case 'stikerwm': case 'wm': case 'swm': case 'stickergifwm': case 'sgifwm': {
                let [teks1, teks2] = text.split`|`
                if (!teks1) throw `reply image/video dengan caption ${prefix + command} teks1|teks2`
                if (!teks2) throw `reply image/video dengan caption ${prefix + command} teks1|teks2`
            	m.reply(mess.wait)
                if (/image/.test(mime)) {
                    let media = await hisoka.downloadMediaMessage(qmsg)
	    ...
	    break
         ...
         break
            ...
            let { toAudio } = require('./lib/converter')
            let audio = await toAudio(media, 'mp4')
            hisoka.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Converted By santui.dev.mp3`}, { quoted : m })
            }
            break
...
case 'play': case 'ytplay': {
                if (!text) throw `Example : ${prefix + command} bondan - bunga`
...
➤ Url : ${anu.url}`,
	    ...
	    case 'ytmp3': case 'ytaudio': {
                ...
                if (!text) throw `Example : ${prefix + command} https://youtube.com/watch?v= 128kbps`
            ...
            case 'ytmp4': case 'ytvideo': {
                ...
                if (!text) throw `Example : ${prefix + command} https://youtube.com/watch?v= 360p`
