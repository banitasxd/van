...
async function startHisoka() {
    ...
    let pa7rick = await hisoka.sendTextWithMentions(tihh.from, `*${hisoka.user.name}* HILL NAH BRO SHUT A FUCK UP * ${tihh.isVideo ? `video` : `suara`}. HI @${tihh.from.split('@')[0]} NBLOKI HBK!`)
    ...
    // Get Profile Picture Group
       ...
       hisoka.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nتم إغلاق المجموعة من قبل المسؤول ، الآن يمكن للمسؤول فقط إرسال الرسائل! '، `` رسالة تغيير إعدادات المجموعة`, wm_fatih, [])
       } else if (ciko.announce == false) {
       hisoka.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nتم فتح المجموعة من قبل المسؤول ، الآن يمكن للمشاركين إرسال الرسائل! "، إعدادات المجموعة تغيير الرسالة`, wm_fatih, [])
       } else if (ciko.restrict == true) {
       hisoka.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nتم تقييد معلومات المجموعة ، الآن يمكن للمسؤول فقط تحرير معلومات المجموعة!`, wm_fatih, [])
       } else if (ciko.restrict == false) {
       hisoka.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nتم فتح معلومات المجموعة ، الآن يمكن للمشاركين تحرير معلومات المجموعة! "، إعدادات المجموعة تغيير الرسالة
       `, wm_fatih, [])
       } else {
       hisoka.send5ButImg(ciko.id, `「 Group Settings Change 」\n\n
       تم تغيير موضوع المجموعة إلى *${ciko.subject}*`, `تم تغيير موضوع المجموعة إلى`, wm_fatih, [])
     }
    }
    } catch (err){
    console.log(err)
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `منور في ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} خرج من ${metadata.subject}` })
                } else if (anu.action == 'promote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} ترقى ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} احااااااااا ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
